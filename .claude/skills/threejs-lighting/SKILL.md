...
three.js has no `ContactShadows` class; that name comes from drei, a React
library. In plain three.js, follow the official `webgl_shadow_contact` example:
render the objects from the ground plane with an orthographic camera into a
render target, blur it, and draw the result on a plane under them.
...
// fromCubeRenderTarget returns a Promise
lightProbe.copy(
  await LightProbeGenerator.fromCubeRenderTarget(renderer, cubeCamera.renderTarget),
);
```
