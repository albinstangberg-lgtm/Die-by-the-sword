...
"#include <opaque_fragment>"; // Final output (output_fragment in older versions)
...
const material = new THREE.ShaderMaterial({
  ...
  // Extensions. WebGL 2 already has fwidth/dFdx/dFdy, gl_FragDepth, multiple
  // render targets and textureLod, so these are the only flags left:
  extensions: {
    clipCullDistance: false, // vertex shader clipping
    multiDraw: false, // gl_DrawID
  },
...
```javascript
// With Vite, ?raw imports the file as a string (TypeScript needs the
// "vite/client" types for this; a bare .glsl import needs a plugin)
